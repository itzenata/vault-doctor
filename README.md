# Vault Doctor

> Audit + auto-cleanup plugin for Obsidian. **Work in progress.**

## Status

🟡 **Pre-alpha** — design phase, no installable build yet. First mockup is in [`mockup.html`](./mockup.html).

## What it will do

A vault-wide scan producing a health score (0–100) and one-click corrective actions for broken links, orphan notes, duplicates, ghost attachments, and inconsistent tags.

**Hard rules:** local-first (no network), dry-run by default, no real `delete` (system trash only).

## Follow & shape it

- ⭐ Star to be notified when v1 ships
- 💬 [Open an issue](https://github.com/itzenata/vault-doctor/issues/new) describing the vault hygiene problem you'd most like solved — feedback now shapes the rule priorities

License: [MIT](./LICENSE)
